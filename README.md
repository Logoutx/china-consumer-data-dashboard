...
## 备用数据渠道：Kimi Datasource（2026-07-09 记录）

本机的 Kimi Code 命令行工具有一个官方插件 Kimi Datasource，可以用自然语言查询 同花顺（行情、财务数据）和世界银行经济数据库。安装还差一步交互式操作（真实终端里 `kimi` → `/plugins` → Official 标签页 → 选 Kimi Datasource → `/new`）；装好后可脚本化调用 `kimi -p "<查询>"`。

**它在本项目里的定位是交叉核对，不是数据源。** 本看板的数据契约是：只收官方来源（统计局/央行/财政部/海关），原始页面存档进 data/archive/ 且入库前过 Gate A 校验——同花顺是二手转发渠道，不满足这个契约，不能替代任何一条现有抓取管线。合理用法有两种：

1. **数据异常时的旁证**：某序列过不了 Gate A（比如环比跳变超阈值），可以用同花顺快速核对同一指标的第三方转载值，帮助判断是官方修订还是抓取错误。
2. **世界银行数据做国际对比**：如果将来加"中国 vs 其他经济体"的对比板块，世界银行接口是现成的，不用自建抓取。

每次查询消耗 Kimi 账号共享配额（免费档每月 200 次），当旁证工具用，别进定时任务。
